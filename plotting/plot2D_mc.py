...
def main():
	...
	f1 = TFile("DQM_V0001_R000000001__Global__CMSSW_X_Y_Z__RECO_TTbar_PU25ns_102X_upgrade2018_realistic_v12-v1_1kevts_modified.root","OPEN")
	...
	hists = [hist0,hist1]
        names = ["GenericPixelCPE", "TemplatePixelCPE"]
#	runs = [1]
	zHeight = [250, 250]
	...
	for i,hist in enumerate(hists):
		...
		cmsExtra = "Simulation"
		latexCMS.DrawLatex(0.14,0.96,"CMS")
                yLabelPos = 0.96
...
#			yLabelPos = 0.96	
		...
		latexCMSExtra.DrawLatex(0.27,yLabelPos,"%s %s"%(cmsExtra,names[i]))				
#		latexCMSExtra.DrawLatex(0.32,yLabelPos,"%s"%(cmsExtra))				
			

		canv.Print("TrackEtaPhi_%s.pdf"%names[i])
		canv.Print("TrackEtaPhi_%s.png"%names[i])
#		canv.Print("TrackEtaPhi.pdf")
	...
	histSave.Divide(hist1)

	histSave.GetZaxis().SetTitle("N_{tracks} ratio Generic/Template")
	histSave.GetZaxis().SetTitleOffset(1.6)	
	histSave.Draw("colz")
	histSave.GetXaxis().SetRangeUser(-2.5,2.5)
	histSave.GetZaxis().SetRangeUser(0.5,1.5)
	latex = TLatex()
	latex.SetTextFont(42)
	latex.SetTextAlign(31)
	latex.SetTextSize(0.04)
...
#		yLabelPos = 0.96	
	...
	latexCMSExtra.DrawLatex(0.27,yLabelPos,"%s    Generic/Template"%(cmsExtra))		

#	canv.Print("TrackEtaPhi_Ratio_Run%d.pdf"%runs[i])
	canv.Print("TrackEtaPhi_Ratio.pdf")
	canv.Print("TrackEtaPhi_Ratio.png")
